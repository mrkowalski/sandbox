...
## The firewall installs itself on every start

The firewall is installed by the image entrypoint, not by the `devcontainer up`
hook. That means it is reinstalled every time the container starts, however it
was started — `sbx-up`, `docker start`, Docker Desktop's restart button, or an
automatic restart after you reboot. iptables rules do not survive a stop/start,
so without this a restarted container would come back with no firewall at all.
...
**A blocked command here is the sandbox working as designed, not a fault.**
...
If you need the bypass twice, fix the entry instead.

This is an ergonomics feature, not a security boundary: the firewall is what
actually stops these commands from reaching anything, whether or not the guard
runs.
...
Removing it is always safe: the next start re-seeds the volume from the image and re-downloads anything else on demand.
